...
msarima_model<-function(ts,h){
  library(smooth)
  model<-auto.msarima(ts,orders=list(ar=c(1,1),i=c(2,1),ma=c(1,1)))
  model_forecast<-forecast(model,h)
  mape<-mean(abs(ts-model$fitted)/ts)*100
  return (list(model_forecast$mean, model$fitted, mape,"MSARIMA"))
}
...
h=6
model_result<-msarima_model(ts,h)
