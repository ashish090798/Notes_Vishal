df<-read.csv("C:\\Users\\vish\\Documents\\df_c.csv")
head(df)

ets_model<-function(ts,h){
  library(forecast)
  model<-ets(y=ts,lambda=BoxCox.lambda(ts))
  model_forecast<-forecast(model,h)
  mape<-mean(abs(ts-model$fitted)/ts)*100
  return (list(model_forecast$mean, model$fitted, mape,"ETS"))
}
