...
arima_model<-function(ts,h){
  library(forecast)
  model<-auto.arima(y=ts)
  model_forecast=forecast(model,h=h)
  mape<-mean(abs(ts-model$fitted)/ts)*100
  return (list(model_forecast$mean, model$fitted, mape,"Arima"))
}
