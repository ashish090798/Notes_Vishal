...
##till now we converted file data into list and we are dividing data into sentences and pre processed 
##sentences manually using our own function. 
##Now we are converting data into corpus(metrix form) so that we can use tm library to pre process data
##DTM is created when we want to know freq of a term in a document
##DTM is also for a corpus. corpus-> DTM/TDM
##if we have many columns and less row use TDM.
...
##TDM is in existence because having too many columns(10,000) is not preferred so we convert columns into rows 
## i.e DTM into TDM and use it. So if rows are less and columns are high, TDM is used
