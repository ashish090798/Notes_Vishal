...
rf_gridsearch = train(Churn ~., data=train, method=customRF, metric="Accuracy", tuneGrid=tune, trControl=control)
...
gbmFit1 = train(as.factor(Churn) ~ ., data = train, method = "gbm", trControl = fitControl, tuneGrid=tune, verbose = T)
...
pred = predict(gbmtree , newdata = test, n.trees = 1265)
